...
const parseSourceFile = function(caseResultDir, data) {
  ...
  if (source.sessionId) {
    const tree = source.value.tree ? source.value.tree : source.value;
...
exports.dumpXMLAndScreenShot = function *(caseResultId, numberId) {
  ...
  const sourceFilePath = _.getDataPath(caseResultId, numberId).sourceFilePath;
  const screenshotPath = _.getDataPath(caseResultId, numberId).screenshotPath;
