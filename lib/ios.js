...
exports.dumpXMLAndScreenShot = function *(caseResultId, numberId) {
  ...
  _.mkdir(targetDir);
  const sourceDir = "/Users/limeng/";

  const sourceFilePath = path.join(sourceDir, `${caseResultId}`, 'case_log', 'source.txt' );
  const screenshotPath = path.join(sourceDir, `${caseResultId}`, 'screen_shot', 'screenshot.png');
  // logger.info(sourceFilePath);
  const caseResultDir = path.join(targetDir, caseResultId);
  _.mkdir(caseResultDir);
